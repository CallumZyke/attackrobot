...
def serialize(obj):
    if isinstance(obj, Box):
        return {'low': obj.low.tolist(), 'high': obj.high.tolist()}
    return obj
...
def main(cfg):
    ...
    task = cfg.task
    task_k = PARTITION_TO_SPECS["test"][cfg.partition][cfg.task]
    ...
    while True:
        ...
        while True:
            ...
            action_space = {
                'pose0_position': serialize(env.action_space['pose0_position']),
                'pose0_rotation': serialize(env.action_space['pose0_rotation']),
                'pose1_position': serialize(env.action_space['pose1_position']),
                'pose1_rotation': serialize(env.action_space['pose1_rotation'])
            }
            file = "original{}".format(str(elapsed_steps)) + ".json"
            with open(file, 'w') as json_file:
                json.dump(action_space, json_file)
                json.dump(actions, json_file, default=lambda x:x.tolist())
            json_file.close()
            elapsed_steps += 1
            if done:
                break
